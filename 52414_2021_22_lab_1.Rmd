...
```{r}
cross_corr <- function(data_frame,name_of_Country,col_1,col_2) {
data_frame<-subset(data_frame,location ==name_of_Country)
y<- data_frame$col_1
x<-data_frame$col_2
cross_vector <-rep(1:121)
for(j in 60:0){
cross_vector[61-j]<-cor(x[1:(length(x)-j)],y[j+1]:length(y),use = "complete.obs")
}
for(j in 1:61){
cross_vector[60+j]<-cor(y[1:(length(x)-j)],x[j+1]:length(y),use = "complete.obs")
return(cross_vector)
}
}
```
...
```{r}
ggplot(df, aes(x = 60:-60, y = cross_corr(df, "World", "new_cases_smoothed", "new_deaths_smoothed"))) + geom_line()
```

##### Analysis & Results 

From the graph, we can observe the critical time and see the beginning of the decline between the two variables.
