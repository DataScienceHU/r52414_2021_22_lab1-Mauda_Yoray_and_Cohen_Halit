...
n_2<-length(y)
for(j in 60:0){
cross_vector[61-j]<-cor(x[1:(n-j)],y[j+1]:n_2,use = 'complete.obs')
}
for(j in 1:61){
cross_vector[60+j]<-cor(y[1:(n-j)],x[j+1]:n_2,use = 'complete.obs')
}
return(cross_vector)
}

"b"
y_c=cross_corr(df,"World", "new_cases_smoothed", "new_deaths_smoothed")
plot(x = 60:-60, y_C)
```
